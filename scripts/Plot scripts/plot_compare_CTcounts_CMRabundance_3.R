...
png("TimeSeries_LogCTcountsCRabundance_3.png", width = 1200, height=600)
par(mfrow=c(1,2), mar=c(5,7,4,5))

library(gplots)

plotCI(as.Date(time), pormean2_ct$int_2, po_se_ct$int_2, col="skyblue", ylim=c(0,3), ylab="", xlab="", pch=19, cex.main=1.5, cex.lab=1.5,cex.axis=1.5 , cex=2, main="Gray-sided vole", 
                 xaxt='n', yaxt='n', type="p",  lwd=2.5, gap=0)
lines(pormean2_ct$int_2~as.Date(time), lty=2, col="skyblue", lwd=2)

plotCI(as.Date(time)+10, pormean2_cmr$Abundance_HT, po_se_cmr$Abundance_HT, col="orange", ylim=c(0,4), cex.lab=1.3, pch=19, cex=2, add=T, type="p", lwd=2.5, gap=0) 

time2 <- as.Date(time)+10
lines(pormean2_cmr$Abundance_HT ~ (as.Date(time2)), lty=2, col="orange", lwd=2)
...
# Tundra vole
plotCI(time_h,hamean_ct$previousday,se_ct$previousday, col="skyblue", ylim=c(0,3), ylab="", xlab="", pch=19, 
       cex.main=1.5, cex.lab=1.5,cex.axis=1.5 , cex=2, main="Tundra vole", xaxt='n', yaxt='n', type="p", lwd=2.5, gap=0)
lines(hamean_ct$previousday~time_h, lty=2, col="skyblue", lwd=2)

plotCI(time_h+10,hamean_cmr$D50, se_cmr$D50, col="orange", ylim=c(0,3), ylab="CT-index", cex.lab=1.3, pch=19, cex=2, add=T, type="p", lwd=2.5, gap=0) 
lines(hamean_cmr$D50~time_h, lty=2, col="orange", lwd=2)

axis(1, at=as.Date(xlab), lab=c("2018", "2019", "2020"),  cex.axis=1.5)
axis(2, at=c(0,1,2,3,4), lab=c(0,1,2,3,4),  cex.axis=1.5)
legend("topright", legend=c("log(CMR-abundance)","log(CT-index)"), col=c("orange", "skyblue"), pch=c(17,16), cex=1.3)
