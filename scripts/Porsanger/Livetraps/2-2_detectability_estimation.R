...
data4inla0 <- readRDS("data/capture_recapture/porsanger/CHist_porsanger_4INLA.rds")
# dataset with only observed capture histories

data4inla0$weight[is.na(data4inla0$weight)] <- mean(data4inla0$weight, na.rm=TRUE)
data4inla0$sex01[is.na(data4inla0$sex01)] <- 0.5


rawdata <- filter(data4inla0, count==1)
nrow(data4inla0)/3

### filter species here and do this for each species on loop ###
# ------------- Model fitting -------------- #
library(INLA)
#### CR INLA MODEL ####
### MOD 0 ###
...
# Plots
quantile(prob_df$p00_inla, c(.025,0.5,.975))

# boxplot(prob_df[,c(4,5)], ylim=c(.2,.75))

# lump probabilities with individuals

totaldf <- data.frame(rawdata[,c(1:10)],p1.est=prob_df$p1_inla,
...
rawts <- as.data.frame(xtabs(count~trapseason+station, data=rawdata))
rawts2 <- arrange(rawts,station,trapseason)
colnames(rawts2)[3] <- "counts"
rawts2[,c(1,3)] <- as.data.frame(apply(rawts2[,c(1,3)], 2, FUN=as.numeric))

trapdates <- distinct(data4inla0, trapseason,datetime)
...
saveRDS(estimatedf3, "data/capture_recapture/porsanger/porsanger_abundance.rds")
