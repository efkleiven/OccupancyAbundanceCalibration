...
data4inla0 <- readRDS("~/Documents/OccupancyAbundanceCalibration/data/capture_recapture/haakoya/processed/haakoya_inlaformat.rds")
...
totaldf <- data.frame(rawdata[,1:8],p1.est=prob_df$p1_inla,
...
inlaframe <- partialN(p00.st.tp=data.frame(inladf$p00.est,inladf$station,inladf$trapsession))
# add zero counts
...
estimatedf5 <- data.frame(apply(estimatedf3,2,as.numeric))
saveRDS(estimatedf5, "~/Documents/OccupancyAbundanceCalibration/data/capture_recapture/haakoya/processed/haakoya_abundance.rds")
